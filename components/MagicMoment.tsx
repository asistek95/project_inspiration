...
export function MagicMoment() {
  ...
  useEffect(() => {
    if (!previewSrc) setPreviewSrc(SAMPLE_RECEIPTS[0].src);
  }, [previewSrc]);
            ...
            <div className="relative aspect-[3/4] overflow-hidden flex items-center justify-center p-4 lg:p-6">
              {previewSrc ? (
                ...
                <div className="text-center text-slate-400">
                  <ReceiptIcon className="h-16 w-16 opacity-30 mx-auto mb-2" />
                  <p className="text-xs">Beleg wählen oder hochladen</p>
                </div>
              )}
              {state === "scanning" && (
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between mb-3 gap-2 flex-wrap">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
              {SAMPLE_RECEIPTS.length} echte Test-Belege · klicke einen an
            </p>
            <input
              ref={fileRef}
              type="file"
